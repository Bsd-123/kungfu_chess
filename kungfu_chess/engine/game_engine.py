...
def _squares_traveled(src: Position, dst: Position) -> int:
    ...
    sliding/king-like move covers. Spec §10: 'Moving N squares takes
...
class GameEngine:
    ...
    def request_move(self, source: Position, destination: Position) -> MoveResult:
        """Sequential checks, in the exact order mandated by Rule 8/§9:
        ...
        4. If approved: schedule the Motion with the
           ...
           from squares traveled (Spec §10).            -> "ok"
    ...
    def _move_duration_ms(self, piece, source: Position, destination: Position) -> int:
        """duration = squares_traveled * per_square_ms (Spec §10). The
    ...
    # -- Spec §12/§20: read-only snapshot for the rendering boundary ---
    def snapshot(self) -> GameSnapshot:
        """Builds the read-only GameSnapshot DTO handed to a renderer.
        Deliberately does not expose live Board/Piece objects (Spec §12:
        ...
        interpolate an in-flight motion does so from its own animation
        state, not from this snapshot."""
        ...
        for row in range(board.nrows):
            for col in range(board.ncols):
                ...
                pieces.append(PieceSnapshot(
                    kind=piece.kind,
                    color=piece.color,
                    pixel_x=col * self._config.cell_pixel_size,
                    pixel_y=row * self._config.cell_pixel_size,
                    state=piece.state,
                ))
